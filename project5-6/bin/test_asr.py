...
class Solver(BaseSolver):
    ...
    def set_model(self):
        ...
        if self.greedy:
            ...
            if (not self.model.enable_att) or self.config['decode'].get('ctc_weight', 0.0) == 1.0:  #如果只有ctc
                print("only ctc")
                ...
                self.decoder = BeamDecoder(
                    self.model.cpu(), **self.config['decode'])
    ...
    def greedy_decode(self, dv_set):
        ...
        for i, data in enumerate(dv_set):
            ...
            # ================TODO===============
            # get feat, feat_len, txt, txt_len
            # hint：use self.fetch_data()         
            
            # ====================================
            with torch.no_grad():
                ctc_output, encode_len, att_output, att_align, dec_state = \
                    self.decoder(feat, feat_len, 
                                int(float(feat_len.max()) * self.config['decode']['max_len_ratio']))
            
            for j in range(len(txt)):
                idx = j + self.config['data']['corpus']['batch_size'] * i
                
                # ========TODO============
                # Select different decoding methods based on model type
                # If att_output is not None, use Attention decoding
                # Otherwise, use CTC decoding
                # Attention decoding: Take the token with the highest probability at each time step
                # CTC decoding: Take the token with the highest probability at each time step

                # ====================================
                true_txt = txt[j].cpu().tolist()
...
def ctc_beam_decode(data, model, device):
    ...
    return (name[0], hyp, txt[0].cpu().tolist()) # Note: bs == 1
