...
class Solver(BaseSolver):
    ...
    def fetch_data(self, data):
        ...
        feat_len = torch.as_tensor(feat_len, dtype=torch.long, device=self.device)
        txt = txt.to(self.device)
        txt_len = (txt != 0).sum(dim=1).long()
            ...
            with torch.no_grad():
                ctc_output, encode_len, att_output, att_align, dec_state = \
                    self.decoder(feat, feat_len, 
                                int(feat_len.max().item() * self.config['decode']['max_len_ratio']))
            
            for j in range(len(txt)):
                idx = j + self.config['data']['corpus']['batch_size'] * i
                
                if att_output is not None:
                    hyp_seqs = att_output[j].argmax(dim=-1).cpu().tolist()
                else:
                    hyp_seqs = ctc_output[j].argmax(dim=-1).cpu().tolist()
...
def beam_decode(data, model, device):
    ...
    feat = feat.to(device)
    feat_len = torch.as_tensor(feat_len, dtype=torch.long, device=device)
    txt = txt.to(device)
