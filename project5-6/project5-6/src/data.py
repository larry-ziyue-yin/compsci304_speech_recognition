...
from src.audio_pre import create_transform
...
def collect_audio_batch(batch, audio_transform, mode):
    ...
    with torch.no_grad():
        for b in batch:
            # b: (audio_path: str, txt: List[int])
            wav_path, txt_tokens = b

            f_name = str(wav_path)
            feat = audio_transform(f_name)          # [T, D]
            feat_len = feat.shape[0]

            file.append(f_name)
            audio_feat.append(feat)
            audio_len.append(feat_len)
            text.append(torch.LongTensor(txt_tokens))

    # Descending audio length within each batch
    audio_len, file, audio_feat, text = zip(*[
        (feat_len, f_name, feat, txt)
        for feat_len, f_name, feat, txt
        in sorted(zip(audio_len, file, audio_feat, text),
                  reverse=True, key=lambda x: x[0])
    ])

    # Zero-padding
    audio_feat = pad_sequence(audio_feat, batch_first=True)  # [B, T_max, D]
    text = pad_sequence(text, batch_first=True)              # [B, L_max]
...
def create_dataset(tokenizer, ascending, name, path, bucketing, batch_size,
    ...
    if name.lower() == "librispeech":
        ...
        from .aishell import AIShellDataset as Dataset
    else:
...
def create_textset(tokenizer, train_split, dev_split, name, path, bucketing, batch_size):
    ...
    # Recognize corpus
    if name.lower() == "librispeech":
        from .librispeech import LibriTextDataset as Dataset
    elif name.lower() == "aishell":
        from .aishell import AIShellTextDataset as Dataset
    else:
        raise NotImplementedError
