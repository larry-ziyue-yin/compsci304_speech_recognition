...
class ASR(nn.Module):
    ...
    def forward(self, audio_feature, feature_len, decode_step, tf_rate=0.0, teacher=None,
        ...
        # Encode
        # audio_feature: [B, T, D], feature_len: [B]
        encode_feature, encode_len = self.encoder(audio_feature, feature_len)
        # encode_feature: [B, T', H], encode_len: [B]

        # CTC based decoding
        if self.enable_ctc:
            ctc_logits = self.ctc_layer(encode_feature)     # [B, T', V]
            ctc_output = F.log_softmax(ctc_logits, dim=-1)  # [B, T', V]
