...
class BeamDecoder(nn.Module):
    ...
    def __init__(self, asr, beam_size, min_len_ratio, max_len_ratio,
    ...
    def forward(self, audio_feature, feature_len):
        ...
        encode_feature, encode_len = self.asr.encoder(
            audio_feature, feature_len)  #attention encoder输出
        ...
        if self.apply_ctc:
            ...
            if self.ctc_w == 1.0:
                output_seq = ctc_output[0].argmax(dim=-1) #Simply Greedy Decode!
                hypothesis = [Hypothesis(decoder_state=dec_state, output_seq=output_seq, 
                                         output_scores=[0]*len(output_seq), lm_state=None, 
                                         ctc_prob=0,ctc_state=ctc_state, att_map=None)]
                return hypothesis
        ...
        for t in range(max_output_len):
            for hypothesis in prev_top_hypothesis:
                ...
                if self.apply_ctc:
                    # ========TODO CTC Candidate Selection =============
                    # Please select top-K candidate tokens (beam_size candidates)
                    # Hint: Use the topk() method
                ...
                # ========TODO Select Top-K Candidates =============
                # Please select the top beam_size candidates with the highest probabilities
                # Hint: Use the topk() method
            ...
            # ========TODO 13: Hypothesis Sorting =============
            # Please sort the candidate hypotheses in `next_top_hypothesis`, take the top `beam_size` candidates, 
            # update `prev_top_hypothesis`, and clear `next_top_hypothesis`.
            # Hint: Use the sort() method, sorting by avgScore()
