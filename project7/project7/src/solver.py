...
class BaseSolver():
    ...
    def __init__(self, config, paras, mode):
        ...
        if mode == 'train':
            ...
            self.log = SummaryWriter(                                #SummaryWriter() 将条目直接写入 log_dir 中的事件文件以供 TensorBoard 使用。
            ...
            self.verbose('Exp. name : {}'.format(self.exp_name))  #verbose()打印
    ...
    def load_ckpt(self):
        ...
        if self.paras.load:
            # Load weights
            ckpt = torch.load(     #torch.load()加载模型
